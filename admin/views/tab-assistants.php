	...
	<?php if ($delete_assistant): ?>
		...
		<div class="confirm-delete">
			...
			<form method="post">
				...
				<?php submit_button(__('Confirm Delete', 'rds-ai-engine'), 'primary'); ?>
	...
	<?php else: ?>
		...
		<div class="assistant-form">
			...
			<form method="post">
				...
				<input type="hidden" name="assistant_id" value="<?php echo $edit_assistant ? esc_attr($edit_assistant->id) : ''; ?>">

				<table class="form-table">
					<tr>
						<th scope="row">
							<label for="name"><?php _e('Name', 'rds-ai-engine'); ?></label>
						</th>
						<td>
							<input type="text" id="name" name="name" value="<?php echo $edit_assistant ? esc_attr($edit_assistant->name) : ''; ?>" required>
							<p class="description"><?php _e('Display name for this assistant.', 'rds-ai-engine'); ?></p>
						</td>
					</tr>
					<tr>
						<th scope="row">
						...
						<td>
							<textarea id="system_prompt" name="system_prompt" rows="6" class="large-text" required><?php echo $edit_assistant ? esc_textarea($edit_assistant->system_prompt) : ''; ?></textarea>
							<p class="description"><?php _e('Instructions for the assistant behavior.', 'rds-ai-engine'); ?></p>
						</td>
					</tr>
					<tr>
						<th scope="row">
						...
						<td>
							<select id="default_model_id" name="default_model_id" class="regular-text">
								...
								<?php foreach ($models as $model): ?>
									<option value="<?php echo esc_attr($model->id); ?>" <?php selected($edit_assistant ? $edit_assistant->default_model_id : '', $model->id); ?>>
										<?php echo esc_html($model->name); ?> (<?php echo esc_html($model->model_name); ?>)
									</option>
								<?php endforeach; ?>
							</select>
							<p class="description"><?php _e('Default model to use with this assistant.', 'rds-ai-engine'); ?></p>
						</td>
					</tr>
					<tr>
						<th scope="row">
					...
					<tr>
						...
						<td>
							<label>
								...
								<?php _e('Include conversation history in requests', 'rds-ai-engine'); ?>
							</label>
						</td>
					</tr>
					<tr class="history-settings" style="<?php echo ($edit_assistant && $edit_assistant->history_enabled) ? '' : 'display:none;'; ?>">
						<th scope="row">
							<label for="history_messages_count"><?php _e('History Messages Count', 'rds-ai-engine'); ?></label>
						</th>
						<td>
							<input type="number" id="history_messages_count" name="history_messages_count" min="2" max="50" step="1"
								value="<?php echo $edit_assistant ? esc_attr($edit_assistant->history_messages_count) : 10; ?>">
							<p class="description"><?php _e('Number of recent messages to include in history.', 'rds-ai-engine'); ?></p>
						</td>
					</tr>
					<tr>
						<th scope="row">
							<label for="knowledge_base_enabled"><?php _e('Knowledge Base Integration', 'rds-ai-engine'); ?></label>
						</th>
						<td>
							<label>
...
jQuery(document).ready(function($) {
	// Показать/скрыть настройки истории
	$('#history_enabled').change(function() {
		var isChecked = $(this).is(':checked');
		if (isChecked) {
			$('.history-settings').show();
		} else {
			$('.history-settings').hide();
		}
	});
});
</script>